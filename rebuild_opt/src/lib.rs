...
pub fn solve_n_js(puzzle: &str, clue: &str, restarts: u32, steps: u32, n: u32) -> String {
    ...
    // Guarantee n plaintext-distinct candidates: every restart can converge
    // to the same decoding, which used to render as one card.
    let ranked = crate::rerank::diversify(&puz, &ranked, &clues, &cfg, n as usize);
